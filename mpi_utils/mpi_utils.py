...
def sync_grads(network):
    ...
    global_grads = global_grads / num_workers
    _set_flat_params_or_grads(network, global_grads, mode="grads")


def sync_parameter(parameter):
    flat_grads = getattr(parameter, "grad").cpu().numpy().flatten()
    comm = MPI.COMM_WORLD
    num_workers = comm.Get_size()
    global_grads = np.zeros_like(flat_grads)
    comm.Allreduce(flat_grads, global_grads, op=MPI.SUM)
    global_grads = global_grads / num_workers
    pointer = 0
    getattr(parameter, "grad").copy_(
        torch.tensor(global_grads[pointer : pointer + parameter.data.numel()]).view_as(
            parameter.data
        )
    )
