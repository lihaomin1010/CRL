...
class her_sampler:
    ...
    def sample_her_transitions_cross(self, episode_batch, batch_size_in_transitions):
        T = episode_batch["actions"].shape[1]
        rollout_batch_size = episode_batch["actions"].shape[0]
        batch_size = batch_size_in_transitions
        # select which rollouts and which timesteps to be used
        ...
        transitions["neighbor_obs"] = episode_batch["obs"][episode_idxs_2, t_samples].copy()
        transitions["neighbor_ag_next"] = episode_batch["ag_next"][episode_idxs_2, t_samples].copy()
        transitions["neighbor_obs_next"] = episode_batch["obs_next"][episode_idxs_2, t_samples].copy()
        transitions["neighbor_actions"] = episode_batch["actions"][episode_idxs_2, t_samples].copy()
        ...
        if self.labelling_strategy == "proximity":
            ...
            dist = np.linalg.norm(transitions['neighbor_ag_next'] - transitions['g'], axis=1)
